...
export const getGalleryFiltersFromSearch = (search: string | URLSearchParams): GalleryFilterState => {
...
export const applyGalleryFiltersToSearch = (
  ...
  if (filters.tagFilterMode === 'intersection') searchParams.set('tag_mode', 'intersection')
  else searchParams.delete('tag_mode')

  return searchParams
}

export const buildGalleryFilterSearch = (search: string | URLSearchParams, filters: GalleryFilterState): string => {
