...
import { buildGalleryFilterSearch, getGalleryFiltersFromSearch } from '~/lib/gallery-filter-url'

describe('gallery filter URL helpers', () => {
  it('reads gallery filters from URL search params', () => {
    expect(
      getGalleryFiltersFromSearch('?tags=street,night&cameras=SONY+ILCE-7C&lenses=FE+35mm&tag_mode=intersection'),
    ).toEqual({
  ...
  it('removes cleared filters from existing search params', () => {
    expect(
      buildGalleryFilterSearch('?tags=street&cameras=SONY+ILCE-7C&tag_mode=intersection', {
        selectedTags: [],
        selectedCameras: [],
        selectedLenses: [],
        tagFilterMode: 'union',
