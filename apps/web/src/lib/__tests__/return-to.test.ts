...
import {
  buildPhotoDetailSearch,
  getSafeReturnTo,
  syncPhotoDetailSearch,
} from "~/lib/return-to";

describe("return-to helpers", () => {
  it("builds a returnTo query string for internal routes", () => {
    expect(buildPhotoDetailSearch("/explore?photoId=A7C09524")).toBe(
      "?returnTo=%2Fexplore%3FphotoId%3DA7C09524",
    );
  });

  it("reads back safe internal return targets", () => {
    expect(getSafeReturnTo("?returnTo=%2Fexplore%3FphotoId%3DA7C09524")).toBe(
      "/explore?photoId=A7C09524",
    );
  });
  ...
  it("keeps nested explore photoId in sync with the active photo", () => {
    ...
    ).toBe("?returnTo=%2Fexplore%3FphotoId%3DA7C01202");
  });
});
