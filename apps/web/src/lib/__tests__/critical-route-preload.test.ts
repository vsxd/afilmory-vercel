...
describe('critical-route-preload', () => {
  it('waits for the gallery and photo detail route modules', async () => {
    ...
    const loadPhotoDetail = vi.fn().mockResolvedValue({ Component: 'photo-detail' })
  ...
  it('fails bootstrap readiness when a critical route module is missing', () => {
    expect(() =>
      installCriticalRoutePreloads({
        './pages/(main)/layout.tsx': vi.fn().mockResolvedValue({ Component: 'gallery' }),
      }),
    ).toThrow('Missing critical route module: ./pages/(main)/photos/[photoId]/index.tsx')
  })
})
