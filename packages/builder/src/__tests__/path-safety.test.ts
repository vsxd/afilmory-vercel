...
describe("LocalStorageProvider path safety", () => {
  ...
  it("should block sibling directory prefix collisions via getFile (returns null)", async () => {
    ...
    try {
      const result = await provider.getFile(
        `../${path.basename(siblingDir)}/secret.jpg`,
      );
      expect(result).toBeNull();
    } finally {
  ...
  it("should block symlink directory escapes via getFile (returns null)", async () => {
    const outsideDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "afilmory-outside-"),
    );
  ...
  it("should reject path traversal via uploadFile", async () => {
    ...
    await expect(provider.uploadFile("../evil.txt", data)).rejects.toThrow(
      "文件路径不安全",
    );
  });
  ...
  it("should reject sibling directory prefix collisions via uploadFile", async () => {
    ...
    await expect(
      provider.uploadFile(
        `../${path.basename(`${tmpDir}-evil`)}/evil.txt`,
        data,
      ),
    ).rejects.toThrow("文件路径不安全");
  });
  ...
  it("should reject symlink directory escapes via uploadFile", async () => {
    ...
    try {
      await expect(
        provider.uploadFile("linked/evil.txt", Buffer.from("malicious")),
      ).rejects.toThrow("文件路径不安全");
      await expect(
        fs.access(path.join(outsideDir, "evil.txt")),
      ).rejects.toThrow();
    } finally {
      await fs.rm(outsideDir, { recursive: true, force: true });
    }
  ...
  it("should reject sibling directory prefix collisions via deleteFile", async () => {
    await expect(
      provider.deleteFile(`../${path.basename(`${tmpDir}-evil`)}/passwd`),
    ).rejects.toThrow("文件路径不安全");
  });
  ...
  it("should reject symlink directory escapes via deleteFile", async () => {
    ...
    try {
      await expect(provider.deleteFile("linked/secret.txt")).rejects.toThrow(
        "文件路径不安全",
      );
  ...
  it("should encode generated public URLs and avoid file URLs", async () => {
    ...
    await expect(
      providerWithBaseUrl.generatePublicUrl("family/2024 #1?.jpg"),
    ).resolves.toBe("/originals/family/2024%20%231%3F.jpg");
    await expect(provider.generatePublicUrl("sample.jpg")).rejects.toThrow(
      "baseUrl 或 distPath 必须配置",
    );
  });
  ...
  it("should mirror uploads and deletes into distPath before generating public URLs", async () => {
    ...
    try {
      ...
      await expect(
        fs.readFile(path.join(tmpDir, "nested/pic.jpg")),
      ).resolves.toEqual(data);
      await expect(
        fs.readFile(path.join(distPath, "nested/pic.jpg")),
      ).resolves.toEqual(data);
      await expect(
        providerWithDist.generatePublicUrl("nested/pic.jpg"),
      ).resolves.toBe(`/${path.basename(distPath)}/nested/pic.jpg`);

      await providerWithDist.deleteFile("nested/pic.jpg");

      await expect(
        fs.access(path.join(tmpDir, "nested/pic.jpg")),
      ).rejects.toMatchObject({ code: "ENOENT" });
      await expect(
        fs.access(path.join(distPath, "nested/pic.jpg")),
      ).rejects.toMatchObject({ code: "ENOENT" });
    } finally {
