  ...
  try {
        ...
        if (item.Semantic === "MotionPhoto" && item.Length) {
          ...
          if (offset > 0 && offset < rawLength - MIN_VIDEO_SIZE_BYTES) {
            ...
            if (validateMp4Buffer(chunk)) {
              ...
              logger?.warn(
                `[motion-photo] Invalid MP4 at ContainerDirectory offset ${offset}`,
              );
            }
          }
        }
      ...
      if (legacyOffset !== null) {
        ...
        for (const offset of candidates) {
          ...
          if (validateMp4Buffer(chunk)) {
            ...
            logger?.success(
              `[motion-photo] Found video via legacy offset: ${offset}`,
            );
            break;
          }
        }
    ...
    if (videoOffset === null || videoSize === null) {
      if (isMotionPhotoFlag) {
        logger?.warn(
          "[motion-photo] MotionPhoto flag set but no valid video found",
        );
      }
      return null;
    }
